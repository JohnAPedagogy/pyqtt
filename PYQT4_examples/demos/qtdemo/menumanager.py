...
class MenuManager(QtCore.QObject):
    ...
    def getResource(self, name):
        if self.helpEngine:
            ...
            try:
                f = open(os.path.join(os.path.normpath(os.path.join(os.path.dirname(__file__), '..', '..', 'doc')), name[name.find('qdoc/')+5:]), 'r')
            except:
    ...
    def createLeafMenu(self, el):
        ...
        executablePosition = 600
        if self.helpEngine:
            executablePosition = 405
        ...
        if str(el.attribute('executable')) != 'false':
            self.createLowRightLeafButton("Launch", executablePosition, MenuManager.LAUNCH,
